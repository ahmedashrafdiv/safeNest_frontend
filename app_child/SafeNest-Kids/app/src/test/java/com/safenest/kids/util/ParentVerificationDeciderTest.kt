...
class ParentVerificationDeciderTest {
    ...
    @Test
    fun successVerifies() {
        assertEquals(Outcome.VERIFIED, ParentVerificationDecider.outcome(200, null))
    }
    ...
    fun wrongPasswordIsReportedAsSuch() {
        assertEquals(
            ...
            ParentVerificationDecider.outcome(401, "invalid_parent_password"),
        )
    }

    @Test
    fun unauthenticatedDeviceIsNotReportedAsAWrongPassword() {
        assertEquals(Outcome.SESSION_EXPIRED, ParentVerificationDecider.outcome(401, null))
        assertEquals(Outcome.SESSION_EXPIRED, ParentVerificationDecider.outcome(401, "invalid_token"))
    }

    @Test
    fun lockoutIsDistinctFromAWrongPassword() {
        assertEquals(Outcome.LOCKED, ParentVerificationDecider.outcome(429, "verification_locked"))
    }

    @Test
    fun deviceScopeMismatchIsNotAuthorized() {
        assertEquals(Outcome.NOT_AUTHORIZED, ParentVerificationDecider.outcome(403, "device_not_authorized"))
    }

    @Test
    fun missingParentRecordIsAnAccountProblem() {
        assertEquals(Outcome.ACCOUNT_UNAVAILABLE, ParentVerificationDecider.outcome(404, "parent_not_found"))
    }

    @Test
    fun serverErrorsDegradeToUnavailable() {
        assertEquals(Outcome.UNAVAILABLE, ParentVerificationDecider.outcome(500, null))
        assertEquals(Outcome.UNAVAILABLE, ParentVerificationDecider.outcome(502, null))
    ...
    fun aStringDetailFallsBackToTheStatusMapping() {
        ...
        assertEquals(
            ...
            ParentVerificationDecider.outcome(401, ParentVerificationDecider.errorCodeOf(body)),
        )
    }
}
